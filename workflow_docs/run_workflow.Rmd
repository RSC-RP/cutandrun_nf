...
SCRI uses a TLS and/or SSL Certificate is used to encrypt web traffic and its specific to SCRI. Nextflow itself orchestrates many types of downloads from genomic references to container images from public repositories to conda packages. 

Thus it must be configured to use the SCRI certificates. See this [bitbucket repo](https://childrens-atlassian/bitbucket/projects/EC/repos/entcerts-tmp/browse) for the current certificates.

Known Error - cannot download the container images from quay.io on 11/6/2023
> java.lang.IllegalStateException: java.lang.IllegalStateException: Failed to pull singularity image  
  command: apptainer pull  --name quay.io-biocontainers-deeptools-3.5.1--py_0.img.pulling.  1699305035994 docker://quay.io/biocontainers/deeptools:3.5.1--py_0 > /dev/null  
  status : 255  
  message:  
    ...
    Copying blob sha256: [...]  
    FATAL:   While making image from oci registry: error fetching image to cache: while building SIF  from layers: conveyor failed to get: no descriptor found for reference  "5d16e7a95afb816a455df599646ef25335c624b0a0142f4f159d6275a09aa8dc"  

While this error message does not explicely say SSL errors, its the most likely cause due to: 'While trying to use SSH, you may get a response indicating permission is denied. Or if you get an error with a **code of 255**, it means there's a problem with your SSH connection. The command failed with the exit code: 255.'
