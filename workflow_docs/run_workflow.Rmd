...
The directions to set-up the Nextflow workflow requirements are found in the [README.md](../README.md). Ensure that you have followed the steps to fork and clone the repository and created the conda nextflow environment before starting with this document. 
...
**1)** Below is an example of a complete sample sheet for use in the pipeline, which can be edited for your own samples in `test_data/test_dataset_sample_sheet.csv`.
  ...
  * Use `threshold`, and `no_control_macs2` parameters in `nextflow.config` to change this. Details found in [Configure Pipeline for Your Data](#configure-pipeline-for-your-data)
...
**2)** Below is another example of a complete sample sheet for use in the pipeline. 
  ...
  * Use `threshold`, and `no_control_macs2` parameters in `nextflow.config` to modify this. Details found in [Configure Pipeline for Your Data](#configure-pipeline-for-your-data).
...
SEACR defaults to using IgG control normalization and stringent peak calling, eg `SEACR_1.3.sh target_bedgraph igg_bedgraph norm stringent`.

If skipping the use of IgG control all together, set `threshold` to any value > 0. 

If you would like to use a spike-in normalization, either E. Coli or an exongenous spike-in like Drosophila, set `spike_norm` to true. You must see [Advanced Options](#advanced-options) for details on turning off igg normalization. 
...
If you are using a non-model organism or simply don't want to use the effective genome size provided in literature or MACS2 documentation, you can set `calc_effective_gsize = true` to calculate an effective genome size using the target genome fasta `fasta` filepath and read-length. 
...
The **final** outputs are located in the following directories:
...
```{r eval=TRUE, echo = FALSE}
fs::dir_tree("results", recurse = 1)
```

Within each directory you will find the following files (top 5 files per directory are shown):

```{r eval=TRUE, echo = FALSE}
results_dirs <- fs::dir_info("results/GRCh38", recurse = TRUE)
# head(results_dirs)

results_dirs %>% 
    select(path, type) %>% 
    mutate(process = gsub("results/GRCh38/","", path), 
...
# grps <- results_dirs_clean %>% 
#     filter(type == "directory") %>% 
#     select(group, path) %>% 
#     distinct() %>% 
#     pull(group, name = path)
# 
# top_dirs <- purrr::map(grps, function(grp){
#     subset <- results_dirs_clean %>% 
#         filter(group == grp) %>% 
#         select(-group) %>% 
#         as.data.frame() %>% 
#         
#     
#     print(noquote(subset))
# })
```
