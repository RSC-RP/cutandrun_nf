...
head(sample_sheet)
dim(sample_sheet)
table(sample_sheet$sample)
```

Create a small test dataset if needed. Example code is below. 

```{r eval=FALSE}
subset_sheet <- sample_sheet %>%
  filter(grepl("^M[12].+NK", sample_id),
         grepl("K27|K4|IgG", sample_id))

subsample_fqs <- purrr::map(1:nrow(subset_sheet), function(x){
          sampler1 <- ShortRead::FastqSampler(subset_sheet[x,"read1"], 10000)
          set.seed(123)
          R1 <- ShortRead::yield(sampler1)
          sampler2 <- ShortRead::FastqSampler(subset_sheet[x,"read2"], 10000)
          set.seed(123)
          ...
          # ShortRead::writeFastq(R1, file = paste("test_data/Sample_ID",s, ab,"R1.fastq.gz", sep="_"))
          # ShortRead::writeFastq(R2, file = paste("test_data/Sample_ID",s, ab,"R2.fastq.gz",sep="_"))
})


test_sheet <- subset_sheet %>%
...
write.table(test_sheet, "test_data/test_sample_sheet.txt",
